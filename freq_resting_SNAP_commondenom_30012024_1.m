...
occipital_ROI = {'CB1', 'CB2', 'OZ', 'O1', 'O2'};
frontal_ROI   = {'FP1', 'FPZ', 'FP2', 'AF3', 'AF4'};
 %%%
...
data_path = 'M:\\SNAP_Data_Processed\\processed\\bandwith_1'
cd(data_path);
%list = dir('*_pad_bandwidth1.mat'); %change this depending on which bandwidth you are examining  <---- UPDATE
list = dir('*_cleanLine_rmline_pad_bandwidth1.mat'); %change this depending on which bandwidth you are examining  <---- UPDATE
...
% freq_oi   = [45 100];   % frequency range of interest
...
% %group_data_CC.powspctrm_b = group_data_CC.powspctrm./common_denominator_HC;
% %group_data_UHR.powspctrm_b = group_data_UHR.powspctrm./common_denominator_HC;
% Normalize power spectra using mean power over freq range
freq_oi   = [30 45];   % frequency range of interest
freq_norm = [1 100]; % frequency range used to normalize the spectrum
foi_norm  = nearest(group_data_UHR.freq, freq_norm);
common_HC = mean(group_data_HC.powspctrm(:,:,foi_norm(1):foi_norm(2)),3);
common_CC = mean(group_data_CC.powspctrm(:,:,foi_norm(1):foi_norm(2)),3);
common_UHR = mean(group_data_UHR.powspctrm(:,:,foi_norm(1):foi_norm(2)),3);
%Normalise by the MEAN power across all participants within the freq window of interest
mean_allPs_common_HC = mean(common_HC,1);
allPs_common_HC = repmat(mean_allPs_common_HC,size(common_HC,1),1);
%mean_allPs_common_CC = mean(common_CC,1);
allPs_common_CC = repmat(mean_allPs_common_HC,size(common_CC,1),1);
%mean_allPs_common_UHR = mean(common_UHR,1);
allPs_common_UHR = repmat(mean_allPs_common_HC,size(common_UHR,1),1);
% Divide by the cmoon
group_data_HC.powspctrm_b = bsxfun(@rdivide, group_data_HC.powspctrm, allPs_common_HC);     
group_data_CC.powspctrm_b = bsxfun(@rdivide, group_data_CC.powspctrm, allPs_common_CC);
group_data_UHR.powspctrm_b = bsxfun(@rdivide, group_data_UHR.powspctrm, allPs_common_UHR);
...
% Collect data to plot using plotspread function - may need to download from download server linked in
...
figure('position',[680 240 1039 420]); % where the plot is positioned 
...
figure; loglog(group_data_HC.freq,...
...
figure;loglog(group_data_CC.freq,...
...
figure;loglog(group_data_UHR.freq,...
...
%statHCCC = ft_freqstatistics(cfg, group_data_HC, group_data_CC);
%statCCUHR= ft_freqstatistics(cfg, group_data_CC, group_data_UHR);
statUHRCC = ft_freqstatistics(cfg, group_data_HC, group_data_CC, group_data_UHR);


% Plot the results!!!!

cfg = [];
cfg.alpha     = statUHRCC.cfg.alpha;
cfg.parameter = 'stat';
cfg.zlim      = [-3 3];
cfg.elec      = elec;
ft_clusterplot(cfg, statUHRCC);
...
% previously subplots but wasn't working
figure('position',[680 240 1039 420]);
figure; ft_topoplotER(cfg, group_data_HC); colorbar; title('Healthy Control');
%figure; ft_topoplotER(cfg, group_data_CC); colorbar; title('Clinical Control');
figure; ft_topoplotER(cfg, group_data_UHR); colorbar; title('Ultra-High Risk');
